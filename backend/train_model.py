import os
os.environ["KERAS_BACKEND"] = "torch"  # Using PyTorch backend for Keras 3 (compatible with Python 3.14)
...
m_in = layers.Input(shape=(1,))

u_emb = layers.Embedding(num_users, 50)(u_in)
m_emb = layers.Embedding(num_movies, 50)(m_in)

u_vec = layers.Flatten()(u_emb)
m_vec = layers.Flatten()(m_emb)

dot = layers.Dot(axes=1)([u_vec, m_vec])
out = layers.Dense(1, activation='sigmoid')(dot)

model = models.Model([u_in, m_in], out)
...
meta = {
    ...
    "idx2movie": {str(v): int(k) for k, v in movie2idx.items()},
